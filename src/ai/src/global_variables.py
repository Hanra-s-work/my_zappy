...
class ServerData:
    ...
    def __init__(self, ip: str = "0.0.0.0", port: int = 8080) -> None:
        ...
        # f"AI land is live at {self.ip}:{self.port}"
        self.startup_message: str = f"AI client is connecting to {self.ip}:{self.port}"
...
class GlobalVariables:
    ...
    def __init__(self, success: int = 0, error: int = 84, ip: str = "0.0.0.0", port: int = 8080, name: str = "", debug: bool = False) -> None:
        ...
        # server monitoring
        self.tcp_ready: bool = False
        self.tcp_status: int = self.success

        # ai monitoring
        self.ai_ready: bool = False
        self.ai_status: int = self.success

        # Handshake monitoring
        self.initialisation_complete: bool = False

        # Stall if response for the look command is expected
        self.wait_look: bool = False
