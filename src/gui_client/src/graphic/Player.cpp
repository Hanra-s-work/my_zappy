...
Player::Player(const std::string &textureFile, const sf::Vector2f &startPosition, float moveSpeed, float animationSpeed, std::unique_ptr<Sound> &sound)
...
Team::Team(int index, const std::string &textureFile, const sf::Vector2f &startPosition, float moveSpeed, float animationSpeed, std::unique_ptr<Sound> &sound)
...
void Team::updatePosition(std::vector<PlayerState> states)
{
    sf::Vector2f oldDirection = { 0, 0 };
    std::size_t i = 0;

    for (; i < states.size(); i++) {
        if (states[i].id == _index) {
            break;
        }
    }
    if (states[i].old_x == states[i].x && states[i].old_y == states[i].y) {
        _currentDir = Direction::Idle;
        _sound->stopSound("footsteps");
        return;
    }
    ...
    _direction = {0, 0};
    if (states[i].orientation == 1) {
        _direction.y = -1;
        _currentDir = Direction::Up;
        _rect.top = 240;
    } else if (states[i].orientation == 3) {
        _direction.y = 1;
        _currentDir = Direction::Down;
        _rect.top = 160;
    } else if (states[i].orientation == 4) {
        _direction.x = -1;
        _currentDir = Direction::Left;
        _rect.top = 80;
    } else if (states[i].orientation == 2) {
        _direction.x = 1;
        _currentDir = Direction::Right;
        _rect.top = 0;
    }
